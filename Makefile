...
ISO5_DATA_FILES = data/cldr/common/supplemental/languageGroup.xml \
		...
		data/iso639-5_to_glottolog.tsv \
		data/iso639-5_to_iso639-3.tsv
...
data/iso639-5_to_glottolog.tsv:
	rm -f $@
	for l in `wget -qq -O - https://en.wikipedia.org/wiki/List_of_ISO_639-5_codes |\
		grep -B2 'href="/wiki/[^"]*_languages' | \
		tr "\n" "\t" | perl -pe 's/\-\-/\n/g;' | \
		sed 's#^[^<]*<td>\(...\).*href="\(/wiki/[^"]*\)\".*$$#\1=\2#'`; do \
	  echo $$l; \
	  u=`echo "$$l" | cut -f2 -d=`; \
	  echo "$$l" | cut -f1 -d= | tr "\n" "\t" >> $@; \
	  g=`wget -qq -O - https://en.wikipedia.org$$u | \
	  	tr ' ' "\n" | \
	  	egrep -o 'http://glottolog.org/resource/languoid/id/[a-z]{4}[0-9]{4}' | \
	  	sed 's#http.*/##' | tr "\n" "\t" | cut -f1,2`; \
	  echo $$g >> $@; \
	done

data/iso639-5_to_iso639-3.tsv: data/iso639-5_to_glottolog.tsv
	python3 scripts/lookup_glottolog.py < $< > $@
